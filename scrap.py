...
def scrapemore_data(hyperlink):
    try:
        page = requests.get(hyperlink)
        soup = BeautifulSoup(page.content, "html.parser")
        temp_list = []
        for i in soup.find_all("tr",attrs = {"class" : "fact_row"}):
            trtags = i.find_all("td")
            for a in trtags:
                try:
                    temp_list.append(a.find_all("div", attrs={"class": "value"})[0].contents[0])
                except:
                    temp_list.append("")
        new_planet_data.append(temp_list)
    except:
        time.sleep(1)
        scrapemore_data(hyperlink)
scrape()

for index, data in enumerate(planet_data):
    scrapemore_data(data[5])  
